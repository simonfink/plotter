...
<<<<<<< .mine
	...
	static native int isochronousTransfer(ByteBuffer nativeDevice, int endpoint, int iso_packets, byte[] data, int length, int timeout) throws LibusbException;
	
	static native int bulkStreamTransfer(ByteBuffer nativeHandle, byte[] endpoints,  byte num_streams, byte[] data, int length, int timeout) throws LibusbException;
	
	static native int cancelTransfer() throws LibusbException;
||||||| .r343
=======
