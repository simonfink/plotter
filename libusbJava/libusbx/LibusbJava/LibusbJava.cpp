...
<<<<<<< .mine
bool active = false;
libusb_transfer* actualTransfer;
||||||| .r344
=======
bool active = false;
>>>>>>> .r346
 ...
 */
JNIEXPORT void JNICALL Java_ch_ntb_inf_libusb_Libusb_setDebug(JNIEnv *env, jclass obj, jobject ctx, jint level){
 ...
 */
JNIEXPORT jobject JNICALL Java_ch_ntb_inf_libusb_Libusb_init(JNIEnv *env, jclass obj){
	int res;
	libusb_context* context = nullptr;
 ...
 */
JNIEXPORT void JNICALL Java_ch_ntb_inf_libusb_Libusb_exit(JNIEnv *env, jclass obj, jobject ctx){
	libusb_context* context = nullptr;
 ...
 */
JNIEXPORT jobject JNICALL Java_ch_ntb_inf_libusb_Libusb_getDeviceList(JNIEnv *env, jclass obj, jobject ctx){
	libusb_context* context = nullptr;
	ssize_t cnt;
 ...
 */
JNIEXPORT jint JNICALL Java_ch_ntb_inf_libusb_Libusb_controlTransfer(JNIEnv *env, jclass obj, jobject nativeHandle, jbyte requestType, jbyte request, jshort value, jshort index, jbyteArray data, jint length, jint timeout){
	if(!nativeHandle || !data){
		ThrowLibusbException(env, LIBUSB_ERROR_INVALID_PARAM);
		return LIBUSB_ERROR_INVALID_PARAM;
	}
	unsigned char *data_buff = reinterpret_cast<unsigned char*>(env->GetByteArrayElements(data, NULL));
#ifdef DEBUGON
//	cout << "data: " << data_buff << endl;
	...
	for(int i = 0; i < devHandleVector.size(); i++){
		if(devHandleVector[i] == devHandle){
			int retVal = libusb_control_transfer(devHandle, static_cast<uint8_t>(requestType), static_cast<uint8_t>(request), static_cast<uint16_t>(value) , static_cast<uint16_t>(index) , data_buff, static_cast<uint16_t>(length), static_cast<unsigned int>(timeout));
...
JNIEXPORT jint JNICALL Java_ch_ntb_inf_libusb_Libusb_interruptTransfer(JNIEnv *env, jclass obj, jobject nativeHandle, jint endpoint, jbyteArray buffer, jint length, jint timeout){
	if(!nativeHandle || (endpoint < 0) || !buffer){
			ThrowLibusbException(env, LIBUSB_ERROR_INVALID_PARAM);
			return LIBUSB_ERROR_INVALID_PARAM;
		}
	...
	//	cout << "data: " << data << endl;
		...
		for(int i = 0; i < devHandleVector.size(); i++){
			if(devHandleVector[i] == devHandle){
				int retVal = libusb_interrupt_transfer(devHandle, static_cast<uint8_t>(endpoint), data, static_cast<int>(length), &bytes_transferred, static_cast<unsigned int>(timeout));
				env->ReleaseByteArrayElements(buffer, (jbyte*)data, 0);
	//			cout << "retVal: " << retVal << endl;
				if(retVal != 0){
					ThrowLibusbException(env, retVal);
...
static void LIBUSB_CALL callback_iso(struct libusb_transfer *transfer){
 ...
 *///TODO test isochronousTransfer
JNIEXPORT jint JNICALL Java_ch_ntb_inf_libusb_Libusb_isochronousTransfer(JNIEnv *env, jclass obj, jobject nativeDevice, jint iso_packets, jint endpoint, jbyteArray buffer, jint length, jint timeout){
	if(!nativeDevice || !buffer){
		ThrowLibusbException(env, LIBUSB_ERROR_INVALID_PARAM);
		return LIBUSB_ERROR_INVALID_PARAM;
	}
	libusb_device *dev = nullptr;
	...
	libusb_transfer *transfer = libusb_alloc_transfer(static_cast<int>(iso_packets));
	actualTransfer = transfer;
	unsigned short *user_data = 0;
	unsigned char *data = reinterpret_cast<unsigned char*>(env->GetByteArrayElements(buffer, NULL));

	unsigned int iso_pack_length = libusb_get_max_iso_packet_size(dev, static_cast<unsigned char>(endpoint));

	for(int i = 0; i < devHandleVector.size(); i++){
			if(devHandleVector[i] == *devHandle){
					libusb_fill_iso_transfer(transfer, *devHandle, static_cast<unsigned char>(endpoint), data, length, iso_packets, static_cast<libusb_transfer_cb_fn>(callback_iso), &user_data, static_cast<unsigned int>(timeout));
					libusb_set_iso_packet_lengths(transfer, iso_pack_length);
					...
					while(transfer->user_data < (int*)1){}							// submit returns immediately, no answer from device in data
																				// FIXME if transmit is finished, it will call the callback function, howto throw new exception!?
					if(transfer->status == LIBUSB_TRANSFER_COMPLETED){
						...
						ThrowLibusbException(env, LIBUSB_TRANSFER_ERROR);
...
static void LIBUSB_CALL callback_bulkStream(struct libusb_transfer *transfer){
		transfer->user_data = (void*)1;
		libusb_free_transfer(transfer);
}
...
/*
 * Class:     ch_ntb_inf_libusb_Libusb
 * Method:    bulkStreamTransfer
 * Signature: (Ljava/nio/ByteBuffer;I[BII)I
 */// TODO bulkStreamTransfer
JNIEXPORT jint JNICALL Java_ch_ntb_inf_libusb_Libusb_bulkStreamTransfer(JNIEnv *env, jclass obj, jobject nativeHandle, jbyteArray eps, jbyte num_streams, jbyteArray buffer, jint length, jint timeout){
	if(!nativeHandle || !buffer){
		...
		int num_endpoints = env->GetArrayLength(eps);
		unsigned char* endpoints = new unsigned char[num_endpoints];
		env->GetByteArrayRegion(eps, 0, num_endpoints, reinterpret_cast<jbyte*>(endpoints));

		int streams =  libusb_alloc_streams(devHandle, num_streams, endpoints, num_endpoints);		//TODO alloc_streams returns stream-id
		...
		for(int i = 0; i < devHandleVector.size(); i++){
				if(devHandleVector[i] == devHandle){
					for(int i = 0; i < num_endpoints; i++){
						libusb_fill_bulk_stream_transfer(transfer, devHandle, endpoints[i], streams, data, length, callback_bulkStream, user_data, timeout);
					}
					int retVal = libusb_submit_transfer(transfer);
					if(retVal!=0){
						ThrowLibusbException(env, retVal);
 ...
 * Signature: ()I
 */
JNIEXPORT jint JNICALL Java_ch_ntb_inf_libusb_Libusb_cancelTransfer(JNIEnv *env, jclass obj){
	int ret = libusb_cancel_transfer(actualTransfer);
	actualTransfer = NULL;
	return ret;
}
 ...
 */
JNIEXPORT jobject JNICALL Java_ch_ntb_inf_libusb_Libusb_allocTransfer(JNIEnv *env, jclass obj, jint numIsoPacks){
 ...
 */
JNIEXPORT void JNICALL Java_ch_ntb_inf_libusb_Libusb_fillIsoTransfer
  (JNIEnv *env, jclass obj, jobject transfer, jobject nativeHandle, jint endpoint, jbyteArray buffer, jint length, jint numIsoPackets, jstring callbackFunction, jobject user_data, jint timeout, jstring fullQualifiedClassName){
	...
	g_mid = env->GetMethodID(env->FindClass(env->GetStringUTFChars(fullQualifiedClassName, (jboolean*)false)), functionName , "(I)V");


	libusb_device_handle *devHandle = nullptr;
	devHandle = static_cast<libusb_device_handle*>(env->GetDirectBufferAddress(nativeHandle));

	libusb_transfer *transferUsed = static_cast<libusb_transfer*>(env->GetDirectBufferAddress(transfer));
	...
	for(int i = 0; i < devHandleVector.size(); i++){
		if(devHandleVector[i] == devHandle){
			...
			libusb_fill_iso_transfer(transferUsed, devHandle, static_cast<uint8_t>(endpoint), data, static_cast<int>(length), static_cast<int>(numIsoPackets), transferUsed->callback, userData, static_cast<unsigned int>(timeout));
			env->ReleaseByteArrayElements(buffer, (jbyte*)data, 0);
 ...
 */
JNIEXPORT void JNICALL Java_ch_ntb_inf_libusb_Libusb_fillInterruptTransfer(JNIEnv *env, jclass obj, jobject transfer, jobject nativeHandle, jint endpoint, jbyteArray buffer, jint length, jstring callbackFunction, jobject user_data, jint timeout, jstring fullQualifiedClassName){
	if(!nativeHandle || (endpoint < 0) || !buffer){
		ThrowLibusbException(env, LIBUSB_ERROR_INVALID_PARAM);
		return;
	}
	...
	for(int i = 0; i < devHandleVector.size(); i++){
		if(devHandleVector[i] == devHandle){
			...
			libusb_fill_interrupt_transfer(transferUsed, devHandle, static_cast<uint8_t>(endpoint), data, static_cast<int>(length),transferUsed->callback, userData, static_cast<unsigned int>(timeout));
			env->ReleaseByteArrayElements(buffer, (jbyte*)data, 0);
 ...
 * Signature: (Ljava/nio/ByteBuffer;Ljava/nio/ByteBuffer;I[BILjava/lang/String;Ljava/lang/Object;I)I
 */
JNIEXPORT void JNICALL Java_ch_ntb_inf_libusb_Libusb_fillBulkTransfer(JNIEnv *env, jclass obj, jobject transfer, jobject nativeHandle, jint endpoint, jbyteArray buffer, jint length, jstring callbackFunction, jobject user_data, jint timeout, jstring fullQualifiedClassName){
	if(!nativeHandle || (endpoint < 0) || !buffer){
		ThrowLibusbException(env, LIBUSB_ERROR_INVALID_PARAM);
		return;
	}
	...
	for(int i = 0; i < devHandleVector.size(); i++){
		if(devHandleVector[i] == devHandle){
			...
			libusb_fill_bulk_transfer(transferUsed, devHandle, static_cast<uint8_t>(endpoint), data, static_cast<int>(length),transferUsed->callback, userData, static_cast<unsigned int>(timeout));
			env->ReleaseByteArrayElements(buffer, (jbyte*)data, 0);
 ...
 * Signature: (Ljava/nio/ByteBuffer;Ljava/nio/ByteBuffer;I[BILjava/lang/String;Ljava/lang/Object;I)I
 */
JNIEXPORT void JNICALL Java_ch_ntb_inf_libusb_Libusb_fillControlTransfer(JNIEnv *env, jclass obj, jobject transfer, jobject nativeHandle, jint endpoint, jbyteArray buffer, jint length, jstring callbackFunction, jobject user_data, jint timeout, jstring fullQualifiedClassName){
	if(!nativeHandle || (endpoint < 0) || !buffer){
		ThrowLibusbException(env, LIBUSB_ERROR_INVALID_PARAM);
		return;
	}
	...
	for(int i = 0; i < devHandleVector.size(); i++){
		if(devHandleVector[i] == devHandle){
			...
			libusb_fill_control_transfer(transferUsed, devHandle, data, transferUsed->callback, userData, static_cast<unsigned int>(timeout));
			env->ReleaseByteArrayElements(buffer, (jbyte*)data, 0);
...
/*
 * Class:     ch_ntb_inf_libusb_Libusb
 * Method:    fillBulkStreamTransfer
 * Signature: (Ljava/nio/ByteBuffer;Ljava/nio/ByteBuffer;II[BILjava/lang/String;Ljava/lang/Object;I)I
 */
JNIEXPORT void JNICALL Java_ch_ntb_inf_libusb_Libusb_fillBulkStreamTransfer(JNIEnv *env, jclass obj, jobject transfer, jobject nativeHandle, jint endpoint, jint streamID, jbyteArray buffer, jint length, jint numIsoPackets, jstring callbackFunction, jobject user_data, jint timeout, jstring fullQualifiedClassName){
	if(!nativeHandle || (endpoint < 0) || !buffer){
		ThrowLibusbException(env, LIBUSB_ERROR_INVALID_PARAM);
		return;
	}
	...
	g_mid = env->GetMethodID(env->GetObjectClass(obj), env->GetStringUTFChars(callbackFunction, (jboolean*)false), "(I)V");
	...
	for(int i = 0; i < devHandleVector.size(); i++){
		if(devHandleVector[i] == devHandle){
			...
			libusb_fill_bulk_stream_transfer(transferUsed, devHandle, static_cast<unsigned char>(endpoint), static_cast<uint32_t>(streamID), data, static_cast<int>(length), transferUsed->callback, userData, static_cast<unsigned int>(timeout));
			env->ReleaseByteArrayElements(buffer, (jbyte*)data, 0);
 ...
 * Signature: (Ljava/nio/ByteBuffer;)I
 */
JNIEXPORT jint JNICALL Java_ch_ntb_inf_libusb_Libusb_submitTransfer(JNIEnv *env, jclass obj, jobject transfer){
	if(!transfer){
		ThrowLibusbException(env, LIBUSB_ERROR_INVALID_PARAM);
		return -1;
	}
 ...
 * Signature: (Ljava/nio/ByteBuffer;)I
 */
JNIEXPORT jint JNICALL Java_ch_ntb_inf_libusb_Libusb_handleEvents(JNIEnv *env, jclass obj, jobject ctx){
	currentEnv = env;
	currentObj = obj;
	libusb_context* context = nullptr;
 ...
 * Signature: (Ljava/nio/ByteBuffer;)I
 */
JNIEXPORT void JNICALL Java_ch_ntb_inf_libusb_Libusb_freeTransfer(JNIEnv *env, jclass obj, jobject transfer){
	if(!transfer){
		ThrowLibusbException(env, LIBUSB_ERROR_INVALID_PARAM);
		return;
	}
 ...
 */
JNIEXPORT jstring JNICALL Java_ch_ntb_inf_libusb_Libusb_errorName(JNIEnv *env, jclass obj, jint errorName){
 ...
 * Signature: (Ljava/nio/ByteBuffer;I)V
 */
JNIEXPORT void JNICALL Java_ch_ntb_inf_libusb_Libusb_setIsoPacketLength(JNIEnv *env, jclass obj, jobject transfer, jint isoLength){
	if(!transfer){
		ThrowLibusbException(env, LIBUSB_ERROR_INVALID_PARAM);
		return;
	}
