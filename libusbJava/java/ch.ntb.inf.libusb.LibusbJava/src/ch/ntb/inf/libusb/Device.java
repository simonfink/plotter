...
//	private Device parent;
	...
	/**
	 * search a desired device, this has limitations, his has limitations, if you have more 
...
//		releaseInterface(ifaceNum);
	...
	//TODO test interrupt transfers
	/**
	 * NOT TESTED
	 * read/write data to device with an interrupt transfer
	 * @param endpoint
	 * @param data
	 * @param length
	 * @param timeout
	...
	public int interruptTransfer(int endpoint, byte[] data, int length, int timeout) throws LibusbException{
		...
		//TODO interruptTransfer throw LIBUSB_ERROR_TIMEOUT, LIBUSB_ERROR_PIPE, LIBUSB_ERROR_OVERFLOW, LIBUSB_ERROR_NO_DEVICE, another LIBUSB_ERROR
	}
	...
	// TODO isochronuos transfer, receive data or TRANSFER_COMPLETE from another JNIEnv -> callback function from libusb -> asynchronous transfer
	/**
	 * NOT TESTED
	 * read/write data to device with an isochronous transfer
	 * @param endpoint
	 * @param iso_packets
	 * @param data
	 * @param length
	 ...
	 * @throws LibusbException
	 */
	/*public int isochronuosTransfer(int endpoint, int iso_packets, byte[] data, int length, int timeout) throws LibusbException{
		int retVal;
		if(length > data.length){
			throw new InvalidParameterException();
		}
		
		retVal = Libusb.isochronousTransfer(nativeHandle, endpoint, iso_packets, data, length, timeout);
		return retVal;
		
	}
	
	public int readIsochron(int endpoint, int iso_packets, byte[] data, int length, int timeout) throws LibusbException{
		return isochronuosTransfer(endpoint|0x80, iso_packets, data, length, timeout);
	}
	
	public int writeIsochron(int endpoint, int iso_packets, byte[] data, int length, int timeout) throws LibusbException{
		return isochronuosTransfer(endpoint&~(0x80), iso_packets, data, length, timeout);
	}*/
	...
	public void setIsoPacketLength(int isoLength){
		Libusb.setIsoPacketLength(transfer, isoLength);
	}
}
