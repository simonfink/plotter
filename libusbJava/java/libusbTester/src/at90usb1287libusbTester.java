...
public class at90usb1287libusbTester {
	...
	public static void main(String[] args){
		...
		try {
			usbDev = Device.search(useCtx, 0xAFFE, 0x6001);		// 0xAFFE, 0x6001
...
//			List<Interface> ifaceParent = cfgDescParent.getAllInterfaces();
		...
		try {
			...
			res = usbDev.bulkTransfer(3, data, data.length, 0);
			if(res == data.length){
...
//			System.out.println("String descriptor: " + usbDev.getStringDescriptor(serialNumIdx, 0x0409));
		...
		try{
			...
			int res = usbDevice.bulkTransfer(3, dataOut, dataOut.length, 0);
		...
		try{
			...
			usbDevice.claimInterface(0);
			byte[] dataOut = {(byte)0x30, (byte)0x31, (byte)0x32, (byte)0x33};
			System.out.println("Data length 4: " + dataOut.length);
			
			int res = usbDevice.interruptTransfer(4, dataOut, dataOut.length, 0);
			
			if(res == dataOut.length){
				System.out.println("Interrupt transfer successful.");
			}
			else{
			...
			System.out.println("Error occured at interrupt transfer.");
			e.printStackTrace();
		}
		...
		/*try{
			...
			usbDevice.isochronuosTransfer(5, iso_packets, data, length, timeout);
